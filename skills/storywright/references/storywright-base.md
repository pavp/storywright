...
10. **Render** via `references/story-formatter.md`.
    ...
    - **Split/epic folder rule.** The `split` intent writes to `docs/storywright/YYYY-MM-DD-HHmm-epic-<slug>/`, where `<slug>` is the epic title slugged per Rule I, **after stripping a leading `Epic —`, `Epic:`, or `Epic ` prefix (case-insensitive) from the title first.** Anti-stutter rationale: epic titles conventionally read `# Epic — <name>`; slugging that title without stripping the prefix first would produce `<slug> = epic-<name-slug>`, which combined with the `-epic-` infix yields a doubled `…-epic-epic-<name-slug>/` folder name. Strip first, then slug: `# Epic — New dashboard` → stripped `New dashboard` → Rule I → `new-dashboard` → folder `…-epic-new-dashboard/` (no stutter).
