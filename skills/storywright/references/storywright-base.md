...
10. **Render** via `references/story-formatter.md`.
    - Derive the output folder: `storywright/YYYY-MM-DD-HHmm-<title-slug>/` (at the project root) where `YYYY-MM-DD-HHmm` is the current local date+time and `<title-slug>` is the story title slugged per **Rule I** (above).
