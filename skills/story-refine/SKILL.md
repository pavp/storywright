...
- **Detection (Step R, see Application):** classify input into exactly ONE of two paths — **Amendment** or **Plain refine**. The deciding predicate is phrase-agnostic: *is there a concrete requirement whose content is not already inferable from the existing story?* Yes → Amendment. No → Plain refine. An amendment trigger phrase alone (frontmatter `trigger`) is never sufficient — a phrase match with no actual new content beyond the existing story falls through to Plain refine, since there is no delta to merge.
- **Ambiguity fallback:** if the message could be read either way AND the ambiguity affects merge scope, raise ONE `AskUserQuestion` (base rule 1 terminal-only mechanism) to disambiguate, decided case-by-case from the paired story content — never from a static "ambiguous phrases" list. This is a within-Step-R fallback, not a third path.
...
1. Step R (amendment detection — inserted after base step 1, before base step 2): classify the input into Amendment or Plain refine per the detection rule in the Amendment differential above. Do not duplicate the rule here — this is the mandatory numbered gate that decides which path the rest of the skill takes before any language/persona/gap branching runs.
...
- Step 11 (log): label as "Refinement log". On the Amendment path, additionally record: that the run was an amendment, a one-line delta summary, conflict status (and resolution, if any), and whether the estimate changed — within the existing ≤3-line (≤5 if SPLIT) ceiling.
