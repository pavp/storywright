...
13. **Governed source of truth for `story.dev.md`.** The technical detail in `story.dev.md` (endpoints, flags, file paths, CLI commands, library/package names, type/interface names, component names) has exactly two provenances, and the skill MUST declare which:
    - `inferred` — filled from the INPUT and generic domain knowledge ONLY. **HARD PROHIBITION on ALL repo knowledge, from ANY source:**
      - You MUST NOT read, grep, glob, open, or list any workspace file.
      - You MUST NOT use repo facts that are already in your context from another source — the project's `CLAUDE.md`/`AGENTS.md`, files opened earlier in this session, directory listings, env-var names, tool output, or anything the harness injected. If you only know it because this particular repo is open, it does not exist for `inferred`.
      - You MUST NOT name a specific library, package, import path, file path, type/interface, component, env var, or route that did not appear in the user's input. Generic domain patterns are fine ("a data grid component", "a customer type", "the list route"); repo-specific identifiers (`@mui/x-data-grid`, `interface Customer`, `customer-grid.tsx`, `MUI_PRO`, `GridColDef`) are FORBIDDEN — even with a `⚠️ Assumed` tag. `⚠️ Assumed` is for generic gaps, not a loophole to smuggle in repo-derived names.
      - Litmus test before writing any identifier: *"Would I know this if the user had pasted the prompt into a blank editor with no repo open?"* If no → omit it or keep it generic.
      This is the default and the ONLY mode when no workspace is available.
