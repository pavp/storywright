...
8. Render two outputs via `story-formatter` to `storywright/YYYY-MM-DD-HHmm-<title-slug>/` (project root; current local time, title kebab-case max 5 words). Use the `Write` tool for all files — never ask:
