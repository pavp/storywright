...
> Developer supplement to `02-buscar-dashboard.standard.md`. Holds all technical detail stripped from the PM-facing files (rule 3a).

## Technical Considerations
- Búsqueda client-side sobre el resultset ya paginado/filtrado de `01-ver-filtrar-dashboard` (`GET /dashboard/records?filters=<query>`) mediante un `search` param adicional que el mismo endpoint acepta — no es un endpoint nuevo.
...
- Feature flag `dashboard_v2_search`, dependiente de que `dashboard_v2_table` (`01-ver-filtrar-dashboard`) ya esté activo.
...
- **State — usuario limpia el buscador:** volver exactamente al estado de tabla filtrada de `01-ver-filtrar-dashboard`, no al dataset sin filtros.
...
|---|---|---|---|
| `01-ver-filtrar-dashboard` — tabla y filtros del dashboard en producción | Frontend team | IN-PROGRESS | Sí |
...
- [ ] Verificado que la búsqueda respeta los filtros ya aplicados (`01-ver-filtrar-dashboard`), no los ignora
...
| Dependencies | 2 | ×1.5 | 3.0 (sibling adj −1, depende de `01-ver-filtrar-dashboard` ya estimada) |
...
- Independence: PARTIAL · depends on `01-ver-filtrar-dashboard` (rule 10).
