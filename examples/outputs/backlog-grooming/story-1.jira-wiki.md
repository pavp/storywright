h2. Resumen del carrito antes de pagar
...
h3. Business Rules
# El precio mostrado en el resumen es el vigente en el momento de cargar el checkout, no el del momento en que se añadió el artículo al carrito.
# Si el stock de un artículo cae a cero entre que se añadió al carrito y se abre el checkout, el artículo aparece marcado como no disponible e impide continuar al pago.

{panel:title=⚠️ Assumed}
persona = comprador autenticado o invitado que ya tiene artículos en el carrito (auto-inferido del contexto de checkout; confirmar si aplica también a flujo de guest checkout).
{panel}
